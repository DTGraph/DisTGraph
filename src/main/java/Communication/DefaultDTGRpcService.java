 ...
 * @author jiachun.fjc
 */
public class DefaultDTGRpcService implements DTGRpcService {

    private static final Logger            LOG = LoggerFactory.getLogger(DefaultDTGRpcService.class);
    ...
    public DefaultDTGRpcService(DTGPlacementDriverClient pdClient, Endpoint selfEndpoint) {
...
//    }
    ...
    @Override
    public <V> CompletableFuture<V> callAsyncWithRpc(BaseRequest request, raft.FailoverClosure<V> closure, Errors lastCause) {
        return callAsyncWithRpc(request, closure, lastCause, true);
    }

    @Override
    public <V> CompletableFuture<V> callAsyncWithRpc(BaseRequest request, raft.FailoverClosure<V> closure, Errors lastCause, boolean requireLeader) {
        final boolean forceRefresh = ErrorsHelper.isInvalidPeer(lastCause);
        final Endpoint endpoint = getRpcEndpoint(request.getRegionId(), forceRefresh, this.rpcTimeoutMillis,
                requireLeader);
        internalCallAsyncWithRpc(endpoint, request, closure);
