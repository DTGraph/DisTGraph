 ...
 * @author jiachun.fjc
 */
public final class DTGJRaftServiceLoader<S> implements Iterable<S> {
    ...
    public static <S> DTGJRaftServiceLoader<S> load(final Class<S> service) {
        return DTGJRaftServiceLoader.load(service, Thread.currentThread().getContextClassLoader());
    }

    public static <S> DTGJRaftServiceLoader<S> load(final Class<S> service, final ClassLoader loader) {
        return new DTGJRaftServiceLoader<>(service, loader);
    }
    ...
    private DTGJRaftServiceLoader(final Class<S> service, final ClassLoader loader) {
    ...
    public Iterator<S> iterator() {
        return new Iterator<S>() {
            ...
            final Iterator<Map.Entry<String, S>> knownProviders = DTGJRaftServiceLoader.this.providers.entrySet()
                                                                    .iterator();

            @Override
            public boolean hasNext() {
                return this.knownProviders.hasNext() || DTGJRaftServiceLoader.this.lookupIterator.hasNext();
            }
            ...
            public S next() {
                ...
                final Class<S> cls = DTGJRaftServiceLoader.this.lookupIterator.next();
                try {
                    final S provider = DTGJRaftServiceLoader.this.service.cast(cls.newInstance());
                    DTGJRaftServiceLoader.this.providers.put(cls.getName(), provider);
                    return provider;
                } catch (final Throwable x) {
                    throw fail(DTGJRaftServiceLoader.this.service, "provider " + cls.getName()
