 ...
 * @author jiachun.fjc
 */
public class DTGStore implements Copiable<DTGStore>, Serializable {
    ...
    private List<DTGRegion>   regions;                                // list of included regions
    private List<StoreLabel>  labels;                                 // key/value label

    public DTGStore() {
    }

    public DTGStore(long id, Endpoint endpoint, StoreState state, List<DTGRegion> regions, List<StoreLabel> labels) {
        this.id = id;
    ...
    public List<DTGRegion> getRegions() {
        return regions;
    }

    public void setRegions(List<DTGRegion> regions) {
        this.regions = regions;
    }

    @Override
    public DTGStore copy() {
        Endpoint endpoint = null;
        if (this.endpoint != null) {
            endpoint = this.endpoint.copy();
        }
        List<DTGRegion> regions = null;
        if (this.regions != null) {
            regions = Lists.newArrayListWithCapacity(this.regions.size());
            for (DTGRegion region : this.regions) {
        ...
        return new DTGStore(this.id, endpoint, this.state, regions, labels);
    }
