...
//    private int minIdBatchSize;
    ...
    public DTGRegionRouteTable(byte tableType){
...
//                }
                ...
                if(entityEntry.getId() == -2){
                    ...
                    region = findRegionByKeyWithoutLock(toByteArray(entityEntry.getId()));
                }
...
//                    entityEntry.setId(idList.poll());
//                }
                final DTGRegion region = findRegionByKeyWithoutLock(toByteArray(entityEntry.getId()));
                transactionNumRegion.put(entityEntry.getTransactionNum(), region.getId());
                regionMap.computeIfAbsent(region, k -> Lists.newArrayList()).add(entityEntry);
            }
...
//            }
    ...
    public DTGRegion findRegionByEntityEntry(EntityEntry entityEntry){
        return findRegionByKeyWithoutLock(toByteArray(entityEntry.getId()));
    }
    ...
    public void addOrUpdateRegion(final DTGRegion region) {
        ...
        try {
            ...
            for(long[] range : rangeList){
                ...
                this.rangeTable.put(range[0], regionId);
            }
...
//                return this.rangeTable.remove(startKey) != null;
     ...
     */
    public DTGRegion findRegionByKey(final byte[] key) {
    ...
    protected DTGRegion findRegionByKeyWithoutLock(final byte[] key) {
    ...
    protected DTGRegion findRegionByKeyWithoutLock(final long key) {
        // return the greatest key less than or equal to the given key
        final Map.Entry<Long, Long> entry = this.rangeTable.floorEntry(key);
        if (entry == null) {
     ...
     */
    public Map<DTGRegion, List<byte[]>> findRegionsByKeys(final List<byte[]> keys) {
        Requires.requireNonNull(keys, "keys");
        final Map<DTGRegion, List<byte[]>> regionMap = Maps.newHashMap();
        final StampedLock stampedLock = this.stampedLock;
        final long stamp = stampedLock.readLock();
        try {
     ...
     */
    public List<DTGRegion> findRegionsByKeyRange(final byte[] startKey, final byte[] endKey) {
        final StampedLock stampedLock = this.stampedLock;
        final long stamp = stampedLock.readLock();
        try {
...
//    }
    ...
    private List<long[]> getRangeList(DTGRegion region) throws TypeDoesnotExistException {
        switch (this.tableType){
            case NODETYPE :{
                return region.getNodeIdRegionList();
            }
            case RELATIONTYPE:{
                return region.getRelationIdRegionList();
            }
            case TEMPORALPROPERTYTYPE: {
                return region.getTemporalPropertyTimeRegionList();
            }
            default:{
